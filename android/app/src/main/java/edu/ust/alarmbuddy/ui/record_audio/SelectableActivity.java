...
public class SelectableActivity extends AppCompatActivity implements
	...
	private void sendSound() throws InterruptedException {
		...
		if (selectedUsernames.isEmpty()) {
			...
			Toast
				.makeText(this, "Sound name can only contain letters, numbers, and underscores", 10)
				.show();
		} else {
	...
	private void shareSound(String friendName, String soundID, boolean last) {
		...
		String URL = String
			.format("%s/shareSound/%s/%s/%s", AlarmBuddyHttp.API_URL, username, friendName,
				soundID);
		...
		client.newCall(request).enqueue(new Callback() {
			...
			public void onResponse(@NotNull Call call, @NotNull Response response)
				...
				if (response.isSuccessful()) {
					...
					shareSound(selectedUsernames.get(selectedUsernames.size() - 1), soundID[0],
						true);
				} else {
	...
	private void deleteSound(String soundID) {
		...
		String URL = String
			.format("%s/deleteSound/%s/%s", AlarmBuddyHttp.API_URL, username, soundID);
		Request request = new Request.Builder()
			.url(URL)
			.header("Authorization", token)
			.delete()
