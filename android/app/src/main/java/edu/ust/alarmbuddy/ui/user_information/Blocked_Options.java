...
/**
 * Handles the activity that a user is shown when they select a blocked user from their blocked
 * users list. Creates an "Unblock user" button that sends an unblock user request to the database.
 */
public class Blocked_Options extends AppCompatActivity {
	...
	private void Post(String command) throws InterruptedException {
		...
		String url =
			AlarmBuddyHttp.API_URL + "/" + action + "/" + username + "/" + name.getText().toString()
				.trim();
		Log.i(edu.ust.alarmbuddy.ui.user_information.Blocked_Options.class.getName(),
			"URL: " + url);
		...
		client.newCall(request).enqueue(new Callback() {
			...
			public void onResponse(@NotNull Call call, @NotNull Response response)
				throws IOException {
				Log.i(edu.ust.alarmbuddy.ui.friends.Friend_Options.class.getName(),
					"Code: " + response.code());
				Log.i(edu.ust.alarmbuddy.ui.friends.Friend_Options.class.getName(),
	...
	/**
	 * handles the back arrow at the top of the screen that allows the user to go back to the
	 * blocked user list.
	 */
	@Override
