...
var dowload = multer({ dest: 'dowloads/' })
dowload.single('file');
...
app.route('/download/:soundName').get(function(req,res,next) {
  ...
  connection.query("SELECT soundFile FROM alarmbuddy.sounds WHERE soundName = ?", req.params.soundName, function(error, results, fields) {
      if (error) throw error;
      fs.writeFile(__dirname + '/downloads/' + req.params.soundName, results[0].soundFile, function (err) {
        if (error) throw error;
        res.sendFile(__dirname + '/downloads/' + req.params.soundName, (err) => {
          fs.unlinkSync(__dirname + '/downloads/' + req.params.soundName);
        });
      });
    }
  );
